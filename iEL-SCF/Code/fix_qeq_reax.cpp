  ...
  Fix(lmp, narg, arg), pertype_option(NULL)
{
  ...
  tolerance_t = force->numeric(FLERR,arg[6]);
  tolerance_s = force->numeric(FLERR,arg[7]);
  t_s_flag = force->numeric(FLERR,arg[9]);
  iEL_Scf_flag = force->numeric(FLERR,arg[10]); //iEL_Scf_flag=0 no iEL-Scf://if iEL_Scf_flag=1 with Scf
  n1_Scf_flag = force->numeric(FLERR,arg[11]); //if=0 no nScf://if =1 with 1 nScf
  Precon_flag = force->numeric(FLERR,arg[12]); //if=0 no CG Pre conditioning://if =1 with CG Pre conditioning
  i_tolerance_t = force->numeric(FLERR,arg[13]);
  i_tolerance_s = force->numeric(FLERR,arg[14]);
  ...
  dual_enabled = 0;
  if (narg == 16) {
    if (strcmp(arg[10],"dual") == 0) dual_enabled = 1;
  ...
  m_fill = m_cap = 0;
  pack_flag = 0;
  //s = NULL;   // add by Itai for the iEL_ScF
  //t = NULL;   // add by Itai for the iEL_ScF
  nprev = 4;
...
void FixQEqReax::allocate_storage()
{
  ...
  //memory->create(s,nmax,"qeq:s");  // add by Itai for the iEL_ScF
  //memory->create(t,nmax,"qeq:t");  // add by Itai for the iEL_ScF
...
{
  //memory->destroy(s);   // add by Itai for the iEL_ScF
  //memory->destroy(t);   // add by Itai for the iEL_ScF
...
void FixQEqReax::init_storage()
{
  ...
  double *s ;   // add by Itai for the iEL_ScF
  double *t ;    // add by Itai for the iEL_ScF
...
void FixQEqReax::pre_force(int vflag)
{
  ...
  //if(update->ntimestep%100 == 0)printf("n=%i N=%i \n",n,N);
...
void FixQEqReax::init_matvec()
{
  /* fill-in H matrix */
  compute_H();
  double *t_EL_Scf  ;  // add by Itai for the iEL_ScF
  double *s_EL_Scf  ;  // add by Itai for the iEL_ScF
  double *s ;   // add by Itai for the iEL_ScF
  double *t ;   // add by Itai for the iEL_ScF
 ...
 r_last=b_last=q_last=d_last=0.0;
  ...
  if(t_s_flag==0){
    if(iEL_Scf_flag)
      atom->x_last=atom->Chi_eq_iEL_Scf;
      //atom->x_last=x_hist_2 + 3 * ( x_hist_0 - x_hist_1 );  
    else
      atom->x_last=x_hist_2 + 3 * ( x_hist_0 - x_hist_1 );  
      //atom->x_last=x_hist_0;  
  }
  else{
    r_last=b_last=q_last=atom->x_last=d_last=0.0;
  }
...
int FixQEqReax::CG( double *b, double *x)
{
  ...
  if (update->ntimestep == 0) {
    ...
    if (tol_flag==0){
      tolerance_tmp = tolerance_s;
      i_tolerance = i_tolerance_s;
    }
    else{
      tolerance_tmp = tolerance_t;
      i_tolerance = i_tolerance_t;
    }
    
    if (n1_Scf_flag)
      imax_tmp=n1_Scf_flag;
  }

  for (i = 1; (i < imax_tmp && (sqrt(sig_new) / b_norm > tolerance_tmp)) || i < i_tolerance; ++i) {
  //for (i = 1; i < imax_tmp; ++i) {
  ...
  if(update->ntimestep%1000 == 0 and comm->me ==0) printf("Number of SCF iterations= %i\n",i); // added by Itai EL-Scf
...
void FixQEqReax::sparse_matvec( sparse_matrix *A, double *x, double *b, double last)
{
  int i, j, itr_j;
  int nn, NN, ii;
  int *ilist;
...
void FixQEqReax::calculate_Q()
{
  ...
  for (ii = 0; ii < nn; ++ii) {
	...
	t_EL_Scf[i]=t[i];
	if(t_s_flag==0)
	  atom->Chi_eq_iEL_Scf = atom->x_last;
      }
      /* backup s & t */
      for (k = nprev-1; k > 0; --k) {
...
void FixQEqReax::get_names(char *c,double *&ptr)
{
 int index,flag;
 index = atom->find_custom(c,flag);

 if(index!=-1) ptr = atom->dvector[index];
 else error->all(FLERR,"Compute laplacian requires fix property/atom d_loc d_lap command");
}

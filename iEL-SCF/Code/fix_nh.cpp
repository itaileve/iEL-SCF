  ...
  etap_mass(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"temp") == 0) {
      if (iarg+12 > narg) error->all(FLERR,"Illegal fix nvt/npt/nph command");
      tstat_flag = 1;
      ...
      t_s_flag     =force->numeric(FLERR,arg[iarg+4]); //t_s_flag=1:solve for t and s t_s_flag=0 q solve for q (s only)
      iEL_Scf_flag =force->numeric(FLERR,arg[iarg+5]);
      thermo_flag  =force->numeric(FLERR,arg[iarg+6]);
      tautemp_aux  =force->numeric(FLERR,arg[iarg+7]); //100000.0; //t_S //1000000    //100000
      kelvin_aux_t =force->numeric(FLERR,arg[iarg+8]);  //0.000001; //t_S//0.00000001
      kelvin_aux_s =force->numeric(FLERR,arg[iarg+9]); //0.00000001; //t_s//0.0000001  //0.0000001
      Omega_t        =force->numeric(FLERR,arg[iarg+10]);
      Omega_s        =force->numeric(FLERR,arg[iarg+11]);
...
void FixNH::initial_integrate(int vflag)
{
  ...
  if(iEL_Scf_flag){
    ...
    if(t_s_flag==0 ){
      ...
      atom->Chi_eq_iEL_Scf  = atom->Chi_eq_iEL_Scf + atom->vChi_eq_iEL_Scf*dtv;     // add by Itai for the iEL_ScF
    }
...
int FixNH::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
...
void FixNH::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixNH::kinaux (double &temp_aux_t,double &temp_aux_s)
{
  //double *vt_EL_Scf = atom->vt_EL_Scf; // add by Itai for the iEL_ScF
  //double *vs_EL_Scf = atom->vs_EL_Scf; // add by Itai for the iEL_ScF
  double *vt_EL_Scf;
  double *vs_EL_Scf;
  get_names("vt_EL_Scf",vt_EL_Scf); 
  get_names("vs_EL_Scf",vs_EL_Scf); 
...
void FixNH::get_names(char *c,double *&ptr)
{
  int index,flag;
...
void FixNH::Nose_Hoover()
{
  ...
  //if(update->ntimestep%1000==0 and comm->me == 0)
  //  printf("vChi_eq= %.16f temp_s= %.16f v_s[0]= %.16f scale_s= %.16f \n",atom->vChi_eq_iEL_Scf,temp_aux_s,vs_EL_Scf[0],scale_s);
}
